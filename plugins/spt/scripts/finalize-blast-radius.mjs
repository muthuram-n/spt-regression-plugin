...
import { runDir, readJson, writeJson, sha256, arg } from './lib/common.mjs';

const cmd = process.argv[2];
const dir = runDir();
const brFile = path.join(dir, 'blast-radius.json');
const runFile = path.join(dir, 'run.json');
if (!fs.existsSync(brFile)) { console.error('blast-radius.json not found. Run /spt:analyze first.'); process.exit(1); }
...
if (cmd === 'finalise' || cmd === 'finalize') {
  ...
  if (!run.blastRadius?.hash) { console.error('Blast radius not finalised. Review blast-radius.md, then run /spt:finalize.'); process.exit(2); }
  if (run.blastRadius.hash !== hash) { console.error('blast-radius.json changed after it was finalised. Re-run /spt:finalize.'); process.exit(2); }
